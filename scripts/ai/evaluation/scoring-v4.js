/**
 * Deterministic, offline scoring for the QA Agent evaluation Dataset v4
 * (scripts/ai/evaluation/dataset-v4.json).
 *
 * Structurally identical to scoring-v3.js (Dataset v3) - same classification/
 ...
 * Dataset v4 introduces no new scoring dimension; it exists to score two
 * additional post-prompt re-validation samples, not a new sample shape.
 * Kept as its own module (rather than parameterizing scoring-v3.js) for the
 * same reason v1/v2/v3 stay separate: Dataset v3 and Baseline v3 must never
 * be put at risk by a v4 change, and vice versa.
 *
 * Roadmap #13's new sample.metadata provenance fields (revalidationOfExperiment/
 * providerAttempts/firstAttemptError) are deliberately never read here: they
 * are historical facts about how a live observation was produced, not a
 * quality judgment, and are never folded into any aggregate - consistent
 * with the "no composite score" rule the rest of this directory already
 * follows for fabricatedEvidence/policyAdjusted/etc.
 *
 * Scores four things separately, on purpose - never combined into a
 * composite score, for the same reason scoring.js/scoring-v2.js/scoring-v3.js
 * never do:
...
// Same boolean-count treatment as scoring.js/scoring-v2.js/scoring-v3.js -
// fabricatedEvidence is not one of the ternary quality enums, so it is never
// forced into QUALITY_TERNARY_VALUES or the qualitative/correlation
// aggregates.
...
function scoreSampleV4(sample) {
  ...
  // identical rule to scoring.js/scoring-v2.js/scoring-v3.js.
...
// has already run validateDatasetV4() and confirmed dataset.valid.
function evaluateDatasetV4(dataset) {
  const samples = dataset.samples.map(scoreSampleV4);
...
module.exports = { evaluateDatasetV4 };
