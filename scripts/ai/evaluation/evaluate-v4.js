/**
 * Thin CLI wrapper around scoring-v4.js: loads dataset-v4.json from disk,
 * validates it with dataset-v4-schema.js, scores it, and prints a
 * human-readable (or, with --json, machine-readable) report.
 *
 * Mirrors evaluate-v3.js's shape exactly, on purpose - this is the only
 * file in scripts/ai/evaluation/ that touches the filesystem or
 * process.exit/console for Dataset v4, and evaluateDatasetV4() itself stays
...
// Pure formatting: takes the { metrics, samples } shape evaluateDatasetV4()
// returns and produces a plain-text report. No I/O, so it's directly
// testable against fixed metrics objects.
function formatEvaluationSummaryV4({ metrics, samples }) {
  const lines = [];

  lines.push("QA Agent Evaluation — Dataset v4");
  lines.push("");
  lines.push("Samples");
...
function run(datasetPath, options) {
  ...
  const validation = validateDatasetV4(dataset);
  if (!validation.valid) {
    const output = ["Dataset v4 failed validation:", ...validation.errors.map((e) => `  - ${e}`)].join("\n");
    return { exitCode: 1, output };
  }

  const evaluation = evaluateDatasetV4(dataset);
  const output = json ? JSON.stringify(evaluation.metrics, null, 2) : formatEvaluationSummaryV4(evaluation);
...
module.exports = { formatEvaluationSummaryV4, formatPercentage, run };
