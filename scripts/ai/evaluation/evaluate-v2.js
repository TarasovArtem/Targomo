/**
 * Thin CLI wrapper around scoring-v2.js: loads dataset-v2.json from disk,
 * validates it with dataset-v2-schema.js, scores it, and prints a
 * human-readable (or, with --json, machine-readable) report.
 *
 * Mirrors evaluate.js's (Dataset v1) shape exactly, on purpose - this is
 * the only file in scripts/ai/evaluation/ that touches the filesystem or
 * process.exit/console for Dataset v2, and evaluateDatasetV2() itself stays
...
// Pure formatting: takes the { metrics, samples } shape evaluateDatasetV2()
// returns and produces a plain-text report. No I/O, so it's directly
// testable against fixed metrics objects.
function formatEvaluationSummaryV2({ metrics, samples }) {
  const lines = [];

  lines.push("QA Agent Evaluation — Dataset v2");
  lines.push("");
  lines.push("Samples");
...
function run(datasetPath, options) {
  ...
  if (!validation.valid) {
    const output = ["Dataset v2 failed validation:", ...validation.errors.map((e) => `  - ${e}`)].join("\n");
    return { exitCode: 1, output };
  }

  const evaluation = evaluateDatasetV2(dataset);
  const output = json ? JSON.stringify(evaluation.metrics, null, 2) : formatEvaluationSummaryV2(evaluation);
...
module.exports = { formatEvaluationSummaryV2, formatPercentage, run };
