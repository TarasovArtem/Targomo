/**
 * Schema/validator for the frozen QA Agent regression Baseline v4
 * (scripts/ai/evaluation/baseline-v4.json).
 *
 * Separate from baseline-schema.js (v1), baseline-v2-schema.js (v2), and
 * baseline-v3-schema.js (v3) - all three stay untouched. Structurally
 * identical to baseline-v3-schema.js (same per-sample fields:
 * classificationStatus/shouldRetryCorrect/shouldCreateBugCorrect/
 * fabricatedEvidence/rootCause/evidence/recommendedFix/
 * correlationConstruction/correlationTransport/correlationReasoning) with
 * only `datasetVersion` fixed to 4. rootCause/evidence/recommendedFix are
 * mechanically derived from Dataset v4's own curated quality fields - never
 * recurated here. Same "do not duplicate the whole dataset, only freeze the
 * evaluation status" design as v1/v2/v3. Sample-set parity against the
 * current dataset is deliberately NOT checked here - that is
 * regression-v4.js's responsibility.
 *
 * Baseline v4 intentionally does NOT carry the new (Roadmap #13)
 * dataset-v4-only provenance fields (revalidationOfExperiment/
 * providerAttempts/firstAttemptError) - those are historical facts about
 * how a live observation was produced, not part of the frozen regression
 * comparison point, exactly the same reasoning that keeps them out of
 * scoring-v4.js's/regression-v4.js's pass/fail or improvement/regression
 * logic.
 *
 * Pure, synchronous, offline: no filesystem access, no environment
 * variables, no network - same discipline as baseline-v3-schema.js.
 */
...
// and the rootCause/evidence/recommendedFix fields.
...
function validateBaselineV4(baseline) {
  const errors = [];
  ...
  if (baseline.datasetVersion !== 4) {
    errors.push("baseline.datasetVersion: must be 4");
  }
...
module.exports = { validateBaselineV4, CLASSIFICATION_STATUS_VALUES, CORRELATION_QUALITY_VALUES, QUALITY_TERNARY_VALUES };
